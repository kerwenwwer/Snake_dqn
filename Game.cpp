...
void start_game(bool Fullscreen) {
  ...
  if (!display) {
    al_show_native_message_box(display, "Error", "Display Settings",
                               "Couldn't create a display.", NULL,
  ...
  if (!font) {
    al_show_native_message_box(
        display, "Error", "Could not load font file.",
        "Have you included the resources in the same directory of the program?",
  ...
  if (!over_button_sound || !pressed_button_sound) {
    al_show_native_message_box(
        display, "Error", "Could not load one or more sound files.",
  ...
  if (!mouse) {
    al_show_native_message_box(
        display, "Error", "Could not load one or more resource file.",
        "Your resources folder must be corrupt, please download it again.",
        NULL, ALLEGRO_MESSAGEBOX_ERROR);
    exit(-1);
  }
    ...
    switch (gameState) {
      case MainMenu: {
        ...
        if (draw) {
          /* drawing wallpaper */
          if (Fullscreen) {
            al_draw_scaled_bitmap(BigWallpaperBitmap, 0, 0,
                                  al_get_bitmap_width(BigWallpaperBitmap),
                                  al_get_bitmap_height(BigWallpaperBitmap), 0,
                                  0, CurrentScreenWidth, CurrentScreenHeight,
                                  NULL);
          } else {
          ...
          if (play_button.MouseOverButton(mouse_x, mouse_y) &&
            ...
            al_play_sample(over_button_sound, 1.0, 0.0, 1.0,
          ...
          if (play_button.pressed_button && !play_button.StillPressingButton) {
            ...
            al_play_sample(pressed_button_sound, 1.0, 0.0, 1.0,
                           ALLEGRO_PLAYMODE_ONCE, 0);
          } else if (options_button.pressed_button &&
                     !options_button.StillPressingButton) {
            options_button.StillPressingButton = true;
            al_play_sample(pressed_button_sound, 1.0, 0.0, 1.0,
                           ALLEGRO_PLAYMODE_ONCE, 0);
          } else if (exit_button.pressed_button &&
                     !exit_button.StillPressingButton) {
          ...
          al_draw_text(font, White, CurrentScreenWidth / 2,
                       play_button.GetButtonHeightCenter() - 23,
                       ALLEGRO_ALIGN_CENTER, "New Game");
          al_draw_text(font, White, CurrentScreenWidth / 2,
                       options_button.GetButtonHeightCenter() - 23,
                       ALLEGRO_ALIGN_CENTER, fullscreenmode_string.c_str());
          al_draw_text(font, White, CurrentScreenWidth / 2,
                       exit_button.GetButtonHeightCenter() - 23,
                       ALLEGRO_ALIGN_CENTER, "Exit");
          al_draw_text(credits_font, White, 3, CurrentScreenHeight - 20, NULL,
                       "FEUP 2013 - Henrique Ferrolho");
      ...
      case PlayGame: {
        ...
        if (events.type == ALLEGRO_EVENT_TIMER) {
          if (events.timer.source == timer) {
            ...
            if (!snake.IsInScreenBoundaries() || snake.EatedItself()) {
              draw = true;
              al_play_sample(game_over_sound, 1.0, 0.0, 1.0,
                             ALLEGRO_PLAYMODE_ONCE, 0);
              gameState = GameOver;
              break;
            }
            ...
            if (snake.EatedApple(apple.GetAppleX(), apple.GetAppleY())) {
              al_play_sample(eating_apple_sound, 0.5, 0.0, 1.0,
                             ALLEGRO_PLAYMODE_ONCE, 0);
              apple.NewApple(snake.GetSnakeCells());
        ...
        if (draw) {
          /* game frame */
          if (Fullscreen) {
            al_draw_rectangle(0, 0, CurrentScreenWidth, CurrentScreenHeight,
                              DarkRed, 20);
          ...
          if (speed_up_anim) {
            if (speed_up_anim_frame < 10) {
              al_draw_text(font, White, CurrentScreenWidth / 2,
                           (CurrentScreenHeight / 2) - 30, ALLEGRO_ALIGN_CENTER,
                           "SPEED UP!");
              speed_up_anim_frame++;
            } else if (speed_up_anim_frame < 20) {
              al_draw_text(font, Yellow, CurrentScreenWidth / 2,
                           (CurrentScreenHeight / 2) - 30, ALLEGRO_ALIGN_CENTER,
                           "SPEED UP!");
              speed_up_anim_frame++;
          ...
          score = ss.str();
          al_draw_text(credits_font, Yellow, 60, 15, ALLEGRO_ALIGN_CENTER,
                       score.c_str());
      ...
      case GameOver: {
        ...
        if (draw) {
          al_draw_filled_rectangle(0, 0, CurrentScreenWidth,
                                   CurrentScreenHeight, DarkRed);
          ...
          score = ss.str();
          al_draw_text(font, Yellow, CurrentScreenWidth / 2,
                       (CurrentScreenHeight / 2) - 80, ALLEGRO_ALIGN_CENTER,
                       score.c_str());
